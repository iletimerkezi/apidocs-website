...
# JSON Post API
If you want to perform SMS sending using the POST method of the HTTP protocol in JSON format, you can follow the guidelines below.
...
+ **id:** This is a unique numeric value that represents the order. You can check the status of your order using this <Badge>id</Badge> value. If your order is scheduled to be sent at a later date, you can cancel your order using the <Badge>id</Badge> value.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>If the date format you sent with the sendDateTime tag is incorrect, this error will be returned. The date format is DD/MM/YYYY HH:MM (Example: 20/01/2022 13:45).</td>
    </tr>
  </tbody>
</table>
